...
def home(request):
    textos = Diario.objects.order_by("created_at")[:3]
...
def dia(request):
    ...
    diarios = Diario.objects.filter(created_at__gte=data_formatada).filter(
        created_at__lte=data_formatada + timedelta(days=1)
    )
...
def excluir_dia(request):
    ...
    diarios = Diario.objects.filter(created_at__gte=dia).filter(
        created_at__lte=dia + timedelta(days=1)
    )
    diarios.delete()
